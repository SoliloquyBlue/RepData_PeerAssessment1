...
## 2. What is mean total number of steps taken per day?
  1. First we calculate the total steps per day using the summarize command of the dplyr package.  (We must load plyr first though.)
...
## 3. What is the average daily activity pattern?
  1. I interpreted this question to mean we should get the average steps per day for each interval, so I'll construct a new data frame.
...
## 4. Imputing missing values
  1. Well, I had to remove all the NAs from the data frame in order to get the summarize function to work.  Now the assignment calls for finding the number of NAs in the data frame, so we start over with a fresh reading of the file.
```{r}
activity <- read.csv('activity.csv', header=TRUE)
...
I then checked it with sum(is.na(activity$steps)) and determined there were 0 NAs left in activity$steps.  Yay!
  ...
  4. Now to make a histogram... but the original activity data frame doesn't have the total steps per day, so we have to redo steps_per_day:
```{r}
...
## 5. Are there differences in activity patterns between weekdays and weekends?
  1. I probably should have made the date and time columns into the POSIX format from the beginning, but I found it difficult to get the steps_by_interval when I tried it.  Good thing I saved one of my early attempts:
